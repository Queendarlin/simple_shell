...
{
	viqu_print_list(viqu_info->viqu_history);
	return (0);
}
...
int viqu_unset_alias(info_t *viqu_info, char *viqu_str)
{
	...
	*viqu_p = 0;
	viqu_ret = viqu_delete_node_at_index(&(viqu_info->viqu_alias),
			viqu_get_node_index(viqu_info->viqu_alias,
				viqu_node_starts_with(viqu_info->viqu_alias, viqu_str, -1)));
	*viqu_p = viqu_c;
	return (viqu_ret);
}
...
int viqu_myalias(info_t *viqu_info)
{
	...
	for (viqu_i = 1; viqu_info->viqu_argv[viqu_i]; viqu_i++)
	{
		viqu_p = viqu_strchr(viqu_info->viqu_argv[viqu_i], '=');
		if (viqu_p)
			viqu_set_alias(viqu_info, viqu_info->viqu_argv[viqu_i]);
		else
			viqu_print_alias(viqu_node_starts_with(viqu_info->viqu_alias,
						viqu_info->viqu_argv[viqu_i], '='));
	}
